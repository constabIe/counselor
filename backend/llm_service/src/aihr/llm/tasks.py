...
SYSTEM_STRUCTURE = (
    ...
    "Всегда возвращай СТРОГО валидный JSON-объект по заданной схеме. "
...
- Не добавляй поля, которых нет в схеме (например, "source").
...
def task_structure_resume(
    ...
    *,
    ...
    content = client.chat(
        ...
        response_json=True,   # ожидаем JSON-объект
        temperature=0.0,
        top_p=1.0,
        max_tokens=2000,
    )
...
SYSTEM_GENERIC_TAGS = (
    "Ты — ассистент по тегированию текста. "
    "Всегда возвращай ТОЛЬКО валидный JSON вида {\"tags\": [\"...\"]}. "
    "Все теги — в ВЕРХНЕМ РЕГИСТРЕ. Без Markdown и лишних комментариев."
)

USER_GENERIC_TAGS_TMPL = """Сгенерируй до {k} наиболее релевантных тегов по тексту ниже.
ТРЕБОВАНИЯ:
- Верни строго JSON: {{"tags": ["...", "..."]}}
- Каждый тег — СТРОКА в ВЕРХНЕМ РЕГИСТРЕ.
- Используй конкретные технологии/инструменты/направления. Без персональных данных.
...
def task_generate_tags(
    ...
    *,
    ...
    content = client.chat(
        messages=[
            {"role": "system", "content": SYSTEM_GENERIC_TAGS},
            {"role": "user", "content": USER_GENERIC_TAGS_TMPL.format(k=k, text=text[:120000])}
        ],
        response_json=True,
        temperature=0.0,
        top_p=1.0,
    ...
    tags = data.get("tags") or []
    if not isinstance(tags, list):
        raise ValueError("LLM returned invalid generic tags payload")
    used_model = getattr(client, "chat_model", None) or (cfg or SciBoxConfig()).chat_model
    return TaskResult(type="tags_generic", payload={"tags": tags}, meta={"model": used_model})


# ---------- 4) Теги для HR-запроса (по предпочтениям) ----------
...
- Каждый тег — СТРОКА в ВЕРХНЕМ РЕГИСТРЕ.
- Обязательно включи ОДИН возрастной диапазон из набора: "18-25", "26-30", "31-45", "46-50", "51-55", "56-60".
- Остальные теги — целевые навыки/технологии/роли/уровни и важные признаки из запроса (департамент, уровень, языки, образование, опыт).
